...
The `home-routes` handler in the `weather-3.routes.home` namespace
...
The `home-page` function will in turn call the `weather-3.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `weather-3.handler` namespace:
...
Request middleware functions are located under the `weather-3.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `weather-3.dev-middleware` namespace found in
